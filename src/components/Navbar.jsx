  ...
  return (
    ...
    >
      <div className="max-w-[1400px] mx-auto px-6 md:px-12 lg:px-16 flex items-center justify-between">
        ...
        >
          {/* Minimal Industrial Mark: Precision Humidity Vector */}
          <span
            className={`text-[17px] md:text-[26px] font-semibold uppercase transition-colors duration-300 ${
          ...
          {navLinks.map((link) => {
            ...
            return (
              <Link
                key={link.name}
                href={link.href}
                className={`text-[15px] lg:text-[16px] tracking-[-0.01em] transition-colors relative py-1 ${
                  isTransparent
                    ? isActive
      ...
      {mobileMenuOpen && (
        <div
          className={`md:hidden border-b px-6 py-8 space-y-6 animate-fadeIn ${
            isTransparent
